...
fn main() {
    ...
    if let Some(v) = args.value_of("attr") {
        ...
        findseq(&paths, v.value_of("subseq").unwrap(), v.is_present("ignore-case"));
        ...
        let n_len = Byte::from_str(subarg.value_of("n_len").unwrap())
            .unwrap()
            .get_bytes() as usize;
        stat(&paths, min_len, genome_len, n_len, subarg.is_present("out_ctg"));
